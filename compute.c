 ...
 */
t_number *
compute(t_bistromathique bistromathique, t_operation_map *operation_map, t_expression_tree **expression_node)
{
    t_number *nb_a = NULL;
    t_number *nb_b = NULL;
    ...
    if ((*expression_node)->first != NULL)
        nb_a = compute(bistromathique, operation_map, &(*expression_node)->first);
    if ((*expression_node)->second != NULL)
        nb_b = compute(bistromathique, operation_map, &(*expression_node)->second);
